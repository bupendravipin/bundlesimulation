...
@app.route('/simulatestub',methods=['GET'])
def recommend_stub():
    productcode=request.args.get('productcode')
    market=request.args.get('market')
    frequency=request.args.get('frequency')
...
@app.route('/businessstub',methods=['GET'])
def business_stub():
    cluster=request.args.get('cluster')
    df_result=get_business(cluster)
    return(jsonify(df_result.to_json(orient='records')))

@app.route('/categorystub',methods=['GET'])
def category_stub():
    cluster=request.args.get('cluster',0)
    bu=request.args.get('business',0)
    df_result=get_category(cluster,bu)
    return(jsonify(df_result.to_json(orient='records')))


@app.route('/productstub',methods=['GET'])
def product_stub():
    cluster=request.args.get('cluster',0)
    bu=request.args.get('business',0)
    category=request.args.get('category',0)
    df_result=get_product(cluster,bu,category)
    return(jsonify(df_result.to_json(orient='records')))

@app.route('/marketstub',methods=['GET'])
def market_stub():
    cluster=request.args.get('cluster',0)
    bu=request.args.get('business',0)
    category=request.args.get('category',0)
    productcode=request.args.get('productcode',0)
    df_result=get_market(cluster,bu,category,productcode)
    return(jsonify(df_result.to_json(orient='records')))
